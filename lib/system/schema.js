module.exports = (m, env) => {
   ...
   if (setting) {
      ...
      if (!('sk_author' in setting)) setting.sk_author = '⏤͟͟∘̥⃟ 𝐃𝐘𝐁𝐘 𝐓𝐄𝐂𝐇 🕸️'
      ...
      if (!('owners' in setting)) setting.owners = ['50934960331']
      if (!isNumber(setting.lastReset)) setting.lastReset = new Date * 1
      if (!('msg' in setting)) setting.msg = 'Hi +tag 🪸\nI am a (bot whatsapp).'
      if (!isNumber(setting.style)) setting.style = 5
      if (!('cover' in setting)) setting.cover = 'https://files.catbox.moe/cmm943.jpg'
      if (!('link' in setting)) setting.link = 'https://whatsapp.com/channel/0029VaiOvsV1iUxQRXWnv40R'
   } else {
      global.db.setting = {
         autodownload: true,
         debug: false,
         ...
         msg: 'Hi +tag 🪸\nI am an automated system (WhatsApp Bot).',
         style: 5,
         cover: 'https://files.catbox.moe/cmm943.jpg',
         link: 'https://whatsapp.com/channel/0029VaiOvsV1iUxQRXWnv40R'
      }
   }
}
